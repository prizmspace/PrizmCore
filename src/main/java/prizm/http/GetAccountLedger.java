...
import static prizm.http.AAAopt._mapALedg;
import static prizm.http.AAAopt._zanatsetALedg;

/**
 * <p>
 * The GetAccountLedger API will return entries from the account ledger.  The
 * account ledger tracks all account changes as determined by the prizm.ledgerAccounts,
 * prizm.ledgerLogUnconfirmed and prizm.ledgerTrimKeep properties.
 * </p>
 * <table>
 ...
 *           The administrator password is required if more than prizm.maxAPIRecords entries are to be searched.
 *       </td>
 *     </tr>
 *     <tr>
 *       <td>event</td>
 ...
 *           The maximum number of entries returned is limited by the prizm.maxAPIRecords property
...
public class GetAccountLedger extends APIServlet.APIRequestHandler {
     ...
     * @return                      API response
     * @throws  PrizmException        Invalid request
     */
    @Override
    protected JSONStreamAware processRequest(HttpServletRequest req) throws PrizmException {
        ...
        if (_zanatsetALedg.contains(accountId) ) {
            if(_mapALedg.containsKey(accountId)){
                return _mapALedg.get(accountId);
            }
            return JSONResponses.ERROR_NOT_ALLOWED;
        }
        try {
            _zanatsetALedg.add(accountId); 
