 ...
 * has not been received within prizm.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by prizm.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Prizm.getIntProperty("prizm.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Prizm.getIntProperty("prizm.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Prizm.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Prizm.getTransactionProcessor();
    ...
    /** Prizm event listeners */
    private final List<PrizmEventListener> PrizmEventListeners = new ArrayList<>();
 ...
 Prizm event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Prizm event registrations
     ...
     *
     * @param   eventRegistrations      Prizm event registrations
     * @throws  EventListenerException  Invalid Prizm event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<PrizmEventListener> it = PrizmEventListeners.iterator();
                while (it.hasNext()) {
                    PrizmEventListener listener = it.next();
                ...
                if (addListener) {
                    PrizmEventListener listener = new PrizmEventListener(event);
                    listener.addListener();
                    PrizmEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void removeEvents(List<EventRegistration> eventRegistrations) {
        ...
        try {
            ...
            //
            if (PrizmEventListeners.isEmpty())
                deactivateListener();
        } finally {
            lock.unlock();
        }
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            PrizmEventListeners.forEach(PrizmEventListener::removeListener);
        } finally {
            lock.unlock();
        }
    ...
    /**
     * Prizm event listener
     */
    private class PrizmEventListener {

        /** Event handler */
        private final PrizmEventHandler eventHandler;

        /**
         * Create the Prizm event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public PrizmEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Prizm listener for this event
         */
        ...
        /**
         * Remove the Prizm listener for this event
         */
        ...
        /**
         * Return the hash code for this Prizm event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Prizm events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof PrizmEventListener) &&
                    eventHandler.equals(((PrizmEventListener)obj).eventHandler));
        }

        /**
         * Prizm listener event handler
         */
        private abstract class PrizmEventHandler {
            ...
            /** Prizm listener event */
            protected final Enum<? extends Enum> event;

            /**
             * Create the Prizm event handler
             *
             * @param   eventRegistration   Event registration
             */
            public PrizmEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof PrizmEventHandler) &&
                                        owner == ((PrizmEventHandler)obj).owner &&
                                        accountId == ((PrizmEventHandler)obj).accountId &&
                                        event == ((PrizmEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends PrizmEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends PrizmEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends PrizmEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends PrizmEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Prizm listener event
        ...
        /**
         * Return the Prizm listener event
         *
         * @return                  Prizm listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
