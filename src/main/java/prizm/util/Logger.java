...
/**
 * Handle logging for the Prizm node server
 */
public final class Logger {
    ...
    static {
        ...
        if (!(LogManager.getLogManager() instanceof PrizmLogManager)) {
        ...
        if (!Boolean.getBoolean("prizm.doNotConfigureLogging")) {
            try {
                ...
                Prizm.loadProperties(loggingProperties, "logging.properties", false);
                Prizm.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(prizm.Prizm.class);
        enableStackTraces = Prizm.getBooleanProperty("prizm.enableStackTraces");
        enableLogTraceback = Prizm.getBooleanProperty("prizm.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof PrizmLogManager) {
            ((PrizmLogManager) LogManager.getLogManager()).prizmShutdown();
        }
    }
