...
import prizm.PrizmException.PrizmIOException;
...
public class CountingInputReader extends FilterReader {
     ...
     * @throws  PrizmIOException      Maximum count exceeded
     */
    private void incCount(long c) throws PrizmIOException {
        count += c;
        if (count > limit)
            throw new PrizmIOException("Maximum size exceeded: " + count);
    }
}
