...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.fixMultiplier = function (multi) {
      return multi * 100;
    }

    NRS.getGoodGrow = function (balance, last, multi) {
        if (multi < 0.0012)
          return NRS.getGoodGrowCompound(balance, last, multi);
        var days = (NRS.getCurrentGlide () - (last * 1000)) / 86400000;
        var grow = balance * days * (multi/100);
        return grow;
    }

    NRS.getGoodGrowCompound = function (balance, last, multi) {
        var periods = (NRS.getCurrentGlide ()/1000 - last) / 50;
        // var grow = balance * periods * (multi/100);``
        var grow = balance * Math.pow(1 + multi, periods) - balance;
        return grow/100;
    }

    NRS.getCurrentGlide = function () {
	  return (new Date ().getTime() - 1532715479000);
    }

    NRS.formatNumberSmart = function (sn) {
        var targetIndex = 2;
        var str = sn.toString();
        if (str.indexOf(".") == -1)
            return str + ".00";
        ...
        if (str.length > targetIndex) {
            return sn.toString().substring(0, sn.toString().indexOf(".") + targetIndex);
        } else {
            return sn.toString();
        }
    }
    ...
    NRS.calculateOrderPricePerWholeQNT = function (price, decimals, returnAsObject) {
		...
		return NRS.convertToPRIZM(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	};
    ...
    NRS.calculateOrderTotal = function (quantityQNT, priceNQT) {
        return NRS.convertToPRIZM(calculateOrderTotalImpl(quantityQNT, priceNQT));
    };
    ...
    NRS.convertToPRIZM = function (amount, returnAsObject) {
        var negative = "";
        var mantissa = "";
        ...
        var fractionalPart = amount.mod(new BigInteger("100")).toString();
        amount = amount.divide(new BigInteger("100"));

        if (fractionalPart && fractionalPart != "0") {
            mantissa = ".";

            for (var i = fractionalPart.length; i < 2; i++) {
                mantissa += "0";
            }
    ...
    NRS.convertToNQT = function (currency) {
		...
		if (parts.length == 1) {
			...
			if (parts[1].length <= 2) {
                ...
                fraction = parts[1].substring(0, 2);
			}
		} else {
			throw $.t("error_invalid_input");
		}

		for (var i = fraction.length; i < 2; i++) {
			fraction += "0";
		}
    ...
    NRS.formatAmount = function (amount, round, no_escaping, zeroPad) {
        ...
        if (typeof amount == "object") {
            var params = NRS.convertToPRIZM(amount, true);
        ...
        if (NRS.settings) {
            ...
            var maxLength = parseInt(NRS.settings.max_prizm_decimals) + offset;
    ...
    NRS.baseTargetPercent = function(block) {
      // var baseTarget = NRS.constants.BASE_TARGET_LEGACY;
      // if (block.height > NRS.constants.BASE_TARGET_TRANSITION_999_END) {
      //   baseTarget = NRS.constants.BASE_TARGET_MODERN;
      // } else if (block.height > NRS.constants.BASE_TARGET_TRANSITION_999_START) {
      //   var stepsCount = NRS.constants.BASE_TARGET_TRANSITION_999_END - NRS.constants.BASE_TARGET_TRANSITION_999_START;
      //   var stepSize = (NRS.constants.BASE_TARGET_MIDDLE - NRS.constants.BASE_TARGET_MODERN) / stepsCount;
      //   var currentStep = block.height - NRS.constants.BASE_TARGET_TRANSITION_999_START;
      //   baseTarget = NRS.constants.BASE_TARGET_MIDDLE - (currentStep * stepSize);
      // } else if (block.height > NRS.constants.BASE_TARGET_TRANSITION_666_END) {
      //   baseTarget = NRS.constants.BASE_TARGET_MIDDLE;
      // } else if (block.height > NRS.constants.BASE_TARGET_TRANSITION_666_START) {
      //   var stepsCount = NRS.constants.BASE_TARGET_TRANSITION_666_END - NRS.constants.BASE_TARGET_TRANSITION_666_START;
      //   var stepSize = (NRS.constants.BASE_TARGET_LEGACY - NRS.constants.BASE_TARGET_MIDDLE) / stepsCount;
      //   var currentStep = block.height - NRS.constants.BASE_TARGET_TRANSITION_666_START;
      //   baseTarget = NRS.constants.BASE_TARGET_LEGACY - (currentStep * stepSize);
      // } else {
      //   baseTarget = NRS.constants.BASE_TARGET_LEGACY;
      // }
      // return Math.round(block.baseTarget / baseTarget * 100)
      var atom = Number(block.baseTarget/NRS.constants.BASE_TARGET_ATOM);
      if (atom / 1000 > 1) {
        return atom.toFixed(1);
      }
      if ((atom / 100) > 1) {
        return atom.toFixed(2);
      }
      if ((atom / 10) > 1) {
        return atom.toFixed(3);
      }
      return atom.toFixed(4);
    };
    ...
    NRS.getAccountLink = function (object, accountKey, accountRef, title, showAccountRS, clazz) {
        ...
        if (accountRS.toUpperCase() === "PRIZM-TE8N-B3VM-JJQH-5NYJB")
            return "GENESIS";
        var accountTitle;
        ...
        return "<a href='#' data-user='" + String(accountRS).escapeHTML() +
            "' id='account_link_in_sidebar' class='show_account_modal_action user-info" + clazz + "'>" + accountTitle + "</a>";
    };
    ...
    NRS.createInfoTable = function (data, fixed) {
		...
		for (var key in data) {
            ...
            if (/_formatted_html$/i.test(key)) {
                ...
                value = NRS.formatAmount(new BigInteger(String(value))) + " PRIZM";
            } else if (key == "sender" || key == "recipient" || key == "account" || key == "seller" || key == "buyer" || key == "lessee") {
                if (value.toUpperCase() === "PRIZM-TE8N-B3VM-JJQH-5NYJB" || value === "8562459348922351959") {
                    value = "GENESIS";
                } else {
            ...
            rows += "<tr><td style='font-weight:bold" + (fixed ? ";width:150px" : "") + "'>" + $.t(key).escapeHTML() + (type ? " " + type.escapeHTML() : "") + ":</td><td style='width:80%;text-transform:none;word-break:break-all'>" + value + "</td></tr>";
        }
  ...
  $("#offcanvas_toggle").on("click", function (e) {
    ...
    // Deprecated
    NRS.copyTextToClipboard = function (text, elem, repeat) {
      var dummyButton = $('#copy-button');
      var dummy = $('#copy-input');
      dummy.val(text);
      var elem2 = $(elem);
      elem2.removeClass("btn-copy");
      elem2.addClass("btn-copy-ok");
      setTimeout(function(){
        elem2.removeClass("btn-copy-ok");
        elem2.addClass("btn-copy");
      }, 500);
      dummyButton.click();
    }

    NRS.createCopyButton = function (textToCopy) {
      if (!ClipboardJS.isSupported()) {
        return "";
      }
      var html = '<button title="'+$.t("copy")+'" type="button" class="btn btn-default btn-copy" onclick="NRS.copyTextToClipboard(\''+textToCopy+'\', this, false)">'+
