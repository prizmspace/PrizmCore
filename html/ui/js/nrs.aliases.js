...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.forms.sellAlias = function ($modal) {
        ...
        if (data.modal == "cancel_alias_sale") {
            ...
            if (data.recipient == NRS.constants.GENESIS_RS) {
                if (!data.pricePRIZM || data.pricePRIZM == "0") {
                    return {
                        "error": $.t("error_not_specified", {
        ...
        if (data.priceNQT == "0") {
            ...
            $.growl(
                $.t("success_alias_sell", {
                    ...
                    "price": NRS.convertToPRIZM(data.priceNQT).escapeHTML()
                }), {
                    "type": "success"
                }
            );
        ...
        NRS.sendRequest("getAlias", {
            ...
            if (response.errorCode) {
                ...
                if (!("priceNQT" in response)) {
                    ...
                    $modal.find("input[name=amountPRIZM]").val(NRS.convertToPRIZM(response.priceNQT)).prop("readonly", true);
                }
            }
        }, false);
    });
    ...
    NRS.forms.buyAliasError = function () {
        $("#buy_alias_modal").find("input[name=pricePRIZM]").prop("readonly", false);
    };
    ...
    NRS.forms.buyAliasComplete = function (response, data) {
        ...
        $.growl(
            $.t("success_alias_buy", {
                "alias_name": data.aliasName,
                "price": NRS.convertToPRIZM(data.amountNQT).escapeHTML()
            }), {
                "type": "success"
            }
        );
        ...
        if (alias) {
            ...
            NRS.sendRequest("getAlias", {
                ...
                if (response.errorCode) {
                    ...
                    } else if ((aliasURI = /acct:(.*)@pzm/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
    ...
    NRS.forms.setAlias = function ($modal) {
        ...
        if (data.type == "account") {
            if (!(/acct:(.*)@pzm/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
                if (/^(PRIZM\-)/i.test(data.aliasURI)) {
                    ...
                    if (!address.set(data.aliasURI)) {
                        ...
                        data.aliasURI = "acct:" + data.aliasURI + "@pzm";
                    }
    ...
    function setAliasType(type, uri) {
        ...
        if (type == "uri") {
            ...
            registerAliasUri.val("").mask("PRIZM-****-****-****-*****");

            if (uri) {
                var match = uri.match(/acct:(.*)@pzm/i);
                if (!match) {
                ...
                } else if (!/^PRIZM\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
                    uri = NRS.accountRS;
                }
                    ...
                    if ("priceNQT" in response) {
                        if (response.buyer == NRS.account) {
                            message = $.t("alias_sale_direct_offer", {
                                "prizm": NRS.formatAmount(response.priceNQT)
