...
var NRS = (function(NRS, $) {
	...
	NRS.addMessageData = function(data, requestType) {
		...
		if (data.add_message && data.message) {
			data.encrypt_message = true; // FIX
			data.add_note_to_self = true;
			data.note_to_self = data.message;
    ...
    NRS.submitForm = function($modal, $btn) {
		...
		$form.find(":input").each(function() {
			if ($(this).is(":invalid")) {
				var error = "";
				var name = String($(this).attr("name")).replace("PRIZM", "").replace("NQT", "").capitalize();
				var value = $(this).val();
    ...
    data.feePRIZM = data.amountPRIZM*0.005 < 0.05 ? 0.05 : ((data.amountPRIZM*0.005) < 10 ? (data.amountPRIZM*0.005) : 10 );
	  data.feePRIZM = isNaN(data.feePRIZM) ? 0.05 : data.feePRIZM;
		...
		if (data.recipient) {
			...
			if (/^\d+$/.test(data.recipient)) {
				...
				NRS.unlockForm($modal, $btn);
				return;
			} else if (!/^PRIZM\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
				var convertedAccountId = $modal.find("input[name=converted_account_id]").val();
				if (!convertedAccountId || (!/^\d+$/.test(convertedAccountId) && !/^PRIZM\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(convertedAccountId))) {
		...
		if (!NRS.showedFormWarning) {
			if ("amountPRIZM" in data && NRS.settings["amount_warning"] && NRS.settings["amount_warning"] != "0") {
				try {
					var amountNQT = NRS.convertToNQT(data.amountPRIZM);
				} catch (err) {
				...
				if (new BigInteger(amountNQT).compareTo(new BigInteger(NRS.settings["amount_warning"])) > 0) {
					...
					$form.find(".error_message").html($.t("error_max_amount_warning", {
						"prizm": NRS.formatAmount(NRS.settings["amount_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			if ("feePRIZM" in data && NRS.settings["fee_warning"] && NRS.settings["fee_warning"] != "0") {
				try {
					var feeNQT = NRS.convertToNQT(data.feePRIZM);
				} catch (err) {
				...
				if (new BigInteger(feeNQT).compareTo(new BigInteger(NRS.settings["fee_warning"])) > 0) {
					...
					$form.find(".error_message").html($.t("error_max_fee_warning", {
						"prizm": NRS.formatAmount(NRS.settings["fee_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			var convertPRIZMFields = ["phasingQuorumPRIZM", "phasingMinBalancePRIZM"];
			$.each(convertPRIZMFields, function(key, field) {
				if (field in data) {
					try {
    ...
    function updateFee(modal, feeNQT) {
        ...
        fee.val(NRS.convertToPRIZM(feeNQT));
        var recalcIndicator = $("#" + modal.attr('id').replace('_modal', '') + "_recalc");
