...
var NRS = (function(NRS, $) {
			...
			if (contacts && contacts.length) {
				...
				$.each(contacts, function (index, contact) {
					...
					rows += "<tr>" +
						...
						"<td style='text-transform:none;word-break:break-all'>" + contactDescription.escapeHTML() + "</td>" +
						"<td style='white-space:nowrap'>" +
						"<a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_money_modal' data-contact='" + String(contact.name).escapeHTML() + "'>" + $.t("send_prizm") + "</a> " +
						"<a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_message_modal' data-contact='" + String(contact.name).escapeHTML() + "'><span class='fa fa-comment' data-toggle='tooltip' title='Message'></span></a> " +
						"<a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#delete_contact_modal' data-contact='" + String(contact.id).escapeHTML() + "'><span class='fa fa-close' data-toggle='tooltip' title='Delete'></span></a>" +
						"</td>" +
					"</tr>";
				});
			}
	...
	NRS.forms.addContact = function($modal) {
		...
		if (/^\d+$/.test(data.name) || /^PRIZM\-/i.test(data.name)) {
			return {
		...
		var address;
		if (/^PRIZM\-/i.test(data.account_id)) {
			data.account_rs = data.account_id;
			address = new PrizmAddress();
	...
	$("#update_contact_modal").on("show.bs.modal", function(e) {
		...
		if (!contactId && NRS.selectedContext) {
			...
			var dbKey = (/^PRIZM\-/i.test(accountId) ? "accountRS" : "account");
			var dbQuery = {};
