...
var NRS = (function(NRS, $) {
	...
	NRS.pages.settings = function() {
		...
		for (var key in NRS.settings) {
            ...
            if (/_warning/i.test(key) && key != "asset_transfer_warning" && key != "currency_transfer_warning") {
				if (setting.length) {
					setting.val(NRS.convertToPRIZM(NRS.settings[key]));
				}
    ...
    if (!key || key == "theme") {
      var s = $("#darkprizm");
      if (NRS.settings["theme"] == 0) {
        if (s.length > 0) {
          s.remove();
          console.log("Enabled day theme");
        }
      } else if (NRS.settings["theme"] == 1) {
        if (s.length > 0) {
    ...
    if (!key || key == "compact_tables") {
      NRS.compactTables = parseInt(NRS.settings["compact_tables"], 10);
    }
