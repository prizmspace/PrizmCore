...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumPRIZM",
			addonText: "PRIZM",
			addonI18n: "prizm_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_prizm', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalancePRIZM';
		context['addonText'] = 'PRIZM';
		context['addonI18n'] = 'prizm_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_prizm', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
