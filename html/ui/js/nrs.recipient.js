...
var NRS = (function(NRS, $) {
	NRS.automaticallyCheckRecipient = function() {
		...
		$recipientFields.on("checkRecipient", function() {
			...
			if (value && value != "PRIZM-____-____-____-_____") {
			...
			if (response.publicKey) {
				if (response.name){
					callback({
						...
						"message": $.t("recipient_info_with_name", {
							"name" : response.name,
							"prizm": NRS.formatAmount(response.unconfirmedBalanceNQT, false, true)
						}),
						"account": response
					});
				}
				...
				if (response.errorCode) {
					if (response.errorCode == 4) {
						callback({
							"type": "danger",
							"message": $.t("recipient_malformed") + (!/^(PRIZM\-)/i.test(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
							"account": null
						});
	...
	NRS.checkRecipient = function(account, modal) {
		...
		if (/^(PRIZM\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {
