...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, isAsync) {
        ...
        //convert PRIZM to NQT...
        var field = "N/A";
        try {
            var pzmFields = [
                ["feePRIZM", "feeNQT"],
                ["amountPRIZM", "amountNQT"],
                ["pricePRIZM", "priceNQT"],
                ["refundPRIZM", "refundNQT"],
                ["discountPRIZM", "discountNQT"],
                ["phasingQuorumPRIZM", "phasingQuorum"],
                ["phasingMinBalancePRIZM", "phasingMinBalance"],
                ["controlQuorumPRIZM", "controlQuorum"],
                ["controlMinBalancePRIZM", "controlMinBalance"],
                ["controlMaxFeesPRIZM", "controlMaxFees"],
                ["minBalancePRIZM", "minBalance"],
                ["shufflingAmountPRIZM", "amount"],
                ["monitorAmountPRIZM", "amount"],
                ["monitorThresholdPRIZM", "threshold"]
            ];

            for (i = 0; i < pzmFields.length; i++) {
                var pzmField = pzmFields[i][0];
                var nqtField = pzmFields[i][1];
                if (pzmField in data) {
                    data[nqtField] = NRS.convertToNQT(data[pzmField]);
                    delete data[pzmField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToPRIZM(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var accountRS = "";
            if (pzmAddress.set(accountId)) {
                accountRS = pzmAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, isAsync) {
        ...
        var url = NRS.server + "/prizm?requestType=" + requestType;
    ...
    NRS.broadcastTransactionBytes = function (transactionData, callback, originalResponse, originalData) {
        $.ajax({
            url: NRS.server + "/prizm?requestType=broadcastTransaction",
            crossDomain: true,
            dataType: "json",
            type: "POST",
            timeout: 30000,
        ...
        NRS.sendRequest("encodeQRCode",
            ...
            function(response) {
                if('qrCodeBase64' in response) {
                    $(target).empty().append(
                        $("<img src='data:image/jpeg;base64,"+response.qrCodeBase64+"'>"+NRS.createCopyButton("https://wallet.prizm.space/?to=" + escape(qrCodeData)))
                    );
                }
            },
            true
    ...
    function addAddressData(data) {
        if (typeof data == "object" && ("recipient" in data)) {
            ...
            if (/^PRIZM\-/i.test(data.recipient)) {
